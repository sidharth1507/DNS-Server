...
def handle_dns_query(request_data, dns_resolver=DEFAULT_DNS_RESOLVER):
    """Handle a DNS query by either forwarding it or responding directly."""
    # Parse the request
    request = DNSMessage.unpack(request_data)
    
    # Check if we have multiple questions
    if request.header.qdcount > 1:
        # Handle multiple questions by splitting them into separate queries
        all_answers = []
        
        for question in request.questions:
            # Create a new request with just this question
            single_question_header = DNSHeader(
            ...
            single_question_message = DNSMessage(
                header=single_question_header,
                questions=[question]
            )
            
            # Forward this single question
            single_response_data = forward_dns_query(single_question_message.pack(), dns_resolver)
            
            if single_response_data:
                # Parse the response
                single_response = DNSMessage.unpack(single_response_data)
                
                # Add the answers to our collection
                all_answers.extend(single_response.answers)
        
        # Create a response with all the answers
...
def main():
    ...
    server_socket.bind(('0.0.0.0', 2054))  # Changed port from 2053 to 2054
    
    print(f"DNS Forwarding Server started on port 2054, forwarding to {dns_resolver[0]}:{dns_resolver[1]}")
    ...
    try:
        while True:
            ...
            response_data = handle_dns_query(request_data, dns_resolver)
